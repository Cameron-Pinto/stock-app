...
def _combine_columns(
    filepaths_by_symbol: Dict[str, str], attr: str = "close"
) -> pd.DataFrame:
    data_frames = [
        pd.read_csv(
            filepath,
            index_col="date",
            usecols=["date", "open", attr, "high", "low", "volume"],
            parse_dates=["date"],
        ).rename(
            columns={
                "date": "date",
...
def load_eod_matrix(tickers: List[str], attr: str = "close") -> pd.DataFrame:
    filepaths_by_symbol = {t: os.path.join(EOD_DATA_DIR, f"{t}.csv") for t in tickers}
